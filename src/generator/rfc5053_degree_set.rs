...
impl RFC5053DegreeSet {
    ...
    pub fn generate_indices(&self, coded_id: usize) -> (Vec<usize>, Vec<usize>) {
        ...
        if a < 1 || a >= self.l_prime as u32 {
            ...
            panic!("the a is invalid, return empty vectors, d={}, a={}, b={}", d, a, b);
        }
        
        // check if b is an integer between 0 and l_prime - 1
        if b >= self.l_prime as u32 {
            panic!("the b is invalid, return empty vectors, d={}, a={}, b={}", d, a, b);
        }
...
mod tests {
    ...
    fn test_degree_set_generation() {
        ...
        for &idx in &active_indices {
            assert!(idx < k + degree_set.l, "Active index {} out of range [0, {})", idx, k + degree_set.l);
        }
        
        // All inactive indices should be within valid range [k+l, L)
        for &idx in &inactive_indices {
            assert!(idx >= k + degree_set.l, "Inactive index {} should be >= {}", idx, k + degree_set.l);
            assert!(idx < degree_set.l, "Inactive index {} out of range [0, {})", idx, degree_set.l);
        }
    }
    ...
    fn test_deterministic_generation() {
        ...
        assert_eq!(active1, active2, "Same coded_id should produce same active indices");
        assert_eq!(inactive1, inactive2, "Same coded_id should produce same inactive indices");
    }
    ...
    fn test_different_coded_ids() {
        ...
        assert!(active1 != active2 || inactive1 != inactive2, 
    ...
    fn test_no_duplicate_indices() {
        ...
        for coded_id in 0..10 {
            ...
            println!("coded_id {}: {} active ({} unique), {} inactive ({} unique)", 
                     coded_id, active_indices.len(), active_unique_count,
                     inactive_indices.len(), inactive_unique_count);
        }
    }
    ...
    fn test_degree_distribution() {
        ...
        println!("Average degree for K={}: {:.2} (gap: [{}, {}))", k, avg_degree, gap_start, gap_end);
    ...
    fn test_degree_set_trait_consistency() {
        ...
        // Should be identical
        assert_eq!(active_via_trait, direct_active, 
                   "Trait method should produce same active indices as direct method");
        assert_eq!(inactive_via_trait, direct_inactive, 
